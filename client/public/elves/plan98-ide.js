...
const $ = elf('plan98-ide', {
  files: []
})

function circus(cwd, nodes) {
  const filesOnly = []
...
fetch(`/plan98/about?cwd=${cwd}`)
  .then(res => res.json())
  .then((about) => {
    const kids = about.plan98.children[0].children
    const files = circus(cwd, kids)
    $.teach({ files })
  })
...
$.draw(target => {
    ...
    target.innerHTML = `
      ...
      <div class="layout">
        <div class="sidebar">
          ...
          <div class="sidebar-inner" data-dom="tree">
          </div>
        </div>
  ...
  afterUpdate: (target) => {
...

    {
      ...
      if(files.length !== target.filesLength) {
        target.filesLength = files.length

        const div = target.querySelector('[data-dom="tree"]')
        diffHTML.innerHTML(div, buildTree(files))
      }
    }
...
function buildTree(files) {
  const tree = { children: [] } // Initialize root tree object

  files.forEach(filePath => { // Renamed to avoid shadowing
    filePath.split('/').reduce((parent, name, i, parts) => {
      if(!parent[name]) {
