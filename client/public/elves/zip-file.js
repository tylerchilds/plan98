...
import { saveZipfile, eventTypes, newDraft, getSearchResults } from './time-machine.js'
...
$.when('click', '[data-new]', (event) => {
  ...
  $.teach({ draft: newDraft(eventTypes.zipfile), ...historyData })
})
...
$.when('json-rpc', 'upload-drop', async (event) => {
...
$.when('click', '[data-download-attachments]', async (event) => {
  ...
  const { data } = zipfiles.find(x => x.data.id === id)
  if(data.attachments) {
