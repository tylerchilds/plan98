...
import("@silly/elf").then((elf) => {
      ...
      return `
        <div class="result" style="--accent: ${accent}">
          <span class="stoplight">
            <button class="upvote" style="--accent: ${accent}" data-key="${key}" aria-label="Vote! ${blurb}" data-tooltip="upvote">
              <sl-icon name="arrow-up-circle-fill"></sl-icon>
            </button>
            <button class="downvote" style="--accent: ${accent}" data-key="${key}" aria-label="Vote! ${blurb}" data-tooltip="downvote">
  ...
  clown.when('click', '.upvote', (event) => {
    const { key } = event.target.dataset
    console.log(event)
    clown.teach({ key, count: 1 }, add)
  })

  clown.when('click', '.downvote', (event) => {
    const { key } = event.target.dataset
    console.log(event)
    clown.teach({ key, count: 1 }, minus)
  })
