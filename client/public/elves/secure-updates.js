...
const $ = elf('secure-updates', { updates: [] })
...
$.draw((target) => {
  ...
  return `
    <form method="POST" action="post-update">
      <textarea data-bind-update name="saga" value="${escapeHyperText(saga)}" style="height: ${$.learn().statusHeight}" placeholder="What's up?"></textarea>
      <button type="submit">
        Post
      </button>
    </form>
...
$.when('input', '[data-bind-update]', (event) => {
