    ...
    <div class="field multiple ${focused ? 'focused': ''}">
      ...
      <div class="selections">
        ${selection ? `
          <span class="tab">
            ${selection.option}
            <button data-remove="${selection.option}">
              x
            </button>
          </span>
        `:''}
...
$.when('keydown', '.query', event => {
  ...
  if(event.keyCode === enter && suggestIndex !== null) {
    event.preventDefault()
    const { suggestions, selection, suggestIndex } = getState(event.target)
    const item = documents(event.target).find(y => {
      return suggestions[suggestIndex].ref === y.option
    })

    if(item) {
      if(!selection || (selection && (selection.option !== item.option))) {
...
$.when('click', '.result', event => {
  event.preventDefault()
  const { selection, suggestions } = getState(event.target)
