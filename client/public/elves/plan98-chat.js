...
if(profile) {
  customArgs = {
    ...
    settingsCustomHyperText: `
      <plan98-profile data-user="${profile}"></plan98-profile>
    `
  }
}

const $ = elf('plan98-chat', {
  ...zeroState,
  ...customArgs,
...
$.draw(target => {
  ...
  if(!loggedIn) {
    return `
      <div data-dom="actions-modal"></div>
...
function afterUpdate(target) {
...

  {
    ...
    recoverElves(target, 'plan98-profile')
    recoverElves(target, 'flying-disk')
  }

  {
    const modal = target.querySelector('[data-dom="actions-modal"]')
    if(modal && settingsModal !== target.settingsModal) {
      target.settingsModal = settingsModal
      innerHTML(modal, renderActions())
    }
  }
