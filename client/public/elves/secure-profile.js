...
const $ = elf('secure-profile', zeroState)
...
const tabs = {
  updates: {
    ...
    render: () => {
      ...
      return `
        <div>
          <secure-updates data-user="${userId}"><secure-updates>
        </div>
      `
    }
  },
  following:  {
    ...
    render: () => {
      return `
        <div>
          <secure-following><secure-following>
        </div>
...
$.draw((target) => {
  ...
  const hero = profile.banner || '/public/cdn/tychi.me/photos/aurora.JPG'
  ...
  afterUpdate: (target) => {
...

    {
      ...
      recoverElves(target, 'secure-updates')
      recoverElves(target, 'secure-following')
