      ...
      <div name="login">
        ...
        <form method="POST" action="loginWithPassword">
          <label class="field">
            ...
            <input data-bind type="text" name="companyName" value="${target.getAttribute('companyName')}" required/>
          </label>
          <label class="field">
            <span class="label">Player</span>
            <input data-bind type="text" name="companyEmployeeId" value="${target.getAttribute('companyEmployeeId')}" required/>
          </label>
          <label class="field">
...
export function __prototypeAuth(companyName, companyEmployeeId, password) {
...
$.style(`
  ...
  & [name="login"] {
    max-width: 320px;
    background: linear-gradient(rgba(255,255,255,.85), lemonchiffon);
    margin: auto;
    inset: 0;
    padding: 1rem;
    position: absolute;
