...
function read($) {
  ...
  const baseline = $.learn()[href] || initial
  return {
    ...initial,
    ...baseline
  }
...
link.draw((target) => {
  if(!target.subscribed) subscribe(link, target)
  const {
    types,
    proposing,
    schedule,
...
link.when('reset', 'form', (event) => {
  write(link, { form: { who: '', whate: '', why: '' }, proposing: false, focused: null })
})


link.when('click', 'button[data-new]', (event) => {
  write(link, { focused: null, proposing: true })
})


link.when('click', 'button[data-insert]', (event) => {
  const { when, where } = event.target.dataset
  write(link, { focused: `${when}-${where}` })
})
...
link.when('change', 'select.cell', (event) => {
  ...
  write(link, { [`${when}-${where}`]: sessionIndex }, (state, payload) => {
    return {
      ...state,
      schedule: {
        ...state.schedule,
